...
stage_label_map = {NEGATIVE:0, ITC:1,MICRO:2,MACRO:3}

# These features are used for training and validation of model
# MODEL_NAME = 'DFCN_121_UNET' 
MODEL_NAME = 'NCRF_CM16' 

training_data = '../predictions/CM17_TrainWSI_Features_{}.csv'.format(MODEL_NAME)

testing_data = '../predictions/CM17_TestWSI_Features_{}.csv'.format(MODEL_NAME)
...
class_names_for_cm = [NEGATIVE, ITC, MICRO, MACRO]
...
def drop_defective_cases(df, feature_list):
    """
    Load Patient information from the csv file as a dataframe
    """
    drop_index = []
    for ind in df.index: 
        if df.loc[ind,feature_list].sum() == 0 and df.loc[ind, 'stage']!=0:
            # print (df.loc[ind, 'patient_name'], df.loc[ind, 'stage'])
            drop_index.append(ind)
    print ('Dropping cases:', len(drop_index))
...
def ModelTester(clf, final_test_path, name, scaler, save_dir='./', label_available=False, prediction_csv=None):
    """
    ...
    target = open(save_dir+'/'+ name+'_predictions_{}.csv'.format(time.strftime("%Y%m%d_%H%M%S")), 'w')
    classes = {NEGATIVE: 0, ITC:0, MICRO:0, MACRO:0}

    line = '{},{}'.format('patient', 'stage')
    target.write(line)
    target.write("\n")

    for pid, pred in zip(df['patient_name'], y_pred):
    ...
    print (classes)
    if label_available:
        y_true,_ = encode_target(df, 'stage', stage_label_map)
        accuracy = accuracy_score(y_true['stage'], y_pred)
        print("Accuracy: %.2f%%" % (accuracy * 100.0))
    else:
        if prediction_csv:
            df = load_dataframe(final_test_path)
            df['stage'] = [class_names[pred] for pred in y_pred]
            df.to_csv(prediction_csv,  index=False)

if __name__ == '__main__':

    save_dir ='../predictions/Report_{}'.format(time.strftime("%Y%m%d_%H%M%S"))
    os.makedirs(save_dir)   
    # Prepare the feature vectors for training and validation
    train_df, train_targets = encode_target(load_dataframe(training_data), 'stage', stage_label_map)

    # Select the features:
    features = list(train_df.columns[np.r_[START_COL:END_COL]])
    print("* features:", features, sep="\n")
    train_df = drop_defective_cases(train_df, features)
    print(len(train_df))
    ...
    plot_confusion_matrix(cnf_matrix, classes=class_names_for_cm,
                          title='Confusion matrix: Random Forest classifier on Training Set')
    # plt.show()
    ...
    train_df, train_targets = encode_target(load_dataframe(training_data, shuffle=False), 'stage', stage_label_map)
    # Select the features:
    features = list(train_df.columns[np.r_[START_COL:END_COL]])
    # print("* features:", features, sep="\n")
    train_df = drop_defective_cases(train_df, features)
    ...
    for clf, label in zip([LR_clf, RF_clf, GNB_clf, XG_clf, SVM_clf, MLP_clf, KNN_clf, E_clf], ['Logistic Regression', 'Random Forest', 'Naive Bayes', 'XG_boost', 'SVM', 'MLP', 'KNN', 'Ensemble']):
        ...
        # print (predictions)
        # print (incorrect_idx)
        # print (train_df['patient_name'][incorrect_idx])
    ...
    RF_clf = RF_clf.fit(X_scaled, y)
    ##################### Results on Final Test data ########################### 
    # No Group/label is available in final test dataset 
    ModelTester(EN_clf, testing_data, name='EnsembleOnFinalTestSet', scaler=scaler, save_dir=save_dir, label_available=False,
                                prediction_csv=None)
    ModelTester(RF_clf, testing_data, name='RFOnFinalTestSet', scaler=scaler, save_dir=save_dir, label_available=False,
                                prediction_csv=None)