...
class WSIStridedPatchDataset(Dataset):
    ...
    def _preprocess(self):
        ...
        if self._label_path is not None:
            self._lbl_obj = openslide.OpenSlide(self._label_path)
...
#         if X_slide != X_mask or Y_slide != Y_mask:
    ...
    def save_scaled_imgs(self):
        ...
        self._slide_scled = self._slide.read_region((0,0),2,scld_dms)
        
        if self._label_path is not None:
            self._lbl_scld = np.array(self._lbl_obj.read_region((0,0),4,scld_dms).convert('L'))
            np.place(self._lbl_scld,self._lbl_scld>0,255)
...
# model_path = glob.glob('../../results/saved_models/incep_viable_80k/5fold_0/sel-model.*')[0]
...
# wsi_paths = glob.glob('../../data/raw-data/train/**/*svs')
wsi_paths = glob.glob('../../data/raw-data/valid/*svs')
out_path = '../../results/saved_imgs/val_8'
...
for i,wsi_path in enumerate(wsi_paths):
    ...
    im_im = np.array(dataset_obj._slide_scled.convert('RGB'))
    st_im = dataset_obj.get_strided_mask()
    ov_im = mask_im/2 + im_im/2
    imsave(ov_im.astype('uint8'),mask_im,(im_im), out=os.path.join(out_path,'mask_'+out_file+'.png'))
    ...
    for i,(data, xes, ys, label) in enumerate(dataloader):
        ...
        for j in range(batch_size):
            ...
            patch_mask = TissueMaskGenerationPatch(image_patches[j]*128+128)
            prd_im_fll[tmp_mns(x):tmp_pls(x),tmp_mns(y):tmp_pls(y)] += pred_map[j,:,:,1]*patch_mask
            # prd_im_fll[tmp_mns(x):tmp_pls(x),tmp_mns(y):tmp_pls(y)] += pred_map[j,:,:,1]
    ...
    prd_im_fll/=count_map
    scaled_count_map = scale(count_map)
    scaled_count_map = scaled_count_map*255//scaled_count_map.max()
    del count_map
    gc.collect()

    prob_map = scale(prd_im_fll)
    prob_map = (prob_map*255).astype('uint8')
    # prd_im_fll = labelthreshold(prd_im_fll,0.8).astype(np.uint8)
    threshold = 0.5
    np.place(prd_im_fll,prd_im_fll>=threshold, 1)
    np.place(prd_im_fll,prd_im_fll<threshold, 0)

    print("Saving to %s" %(out_file))
    tifffile.imsave(os.path.join(out_path,out_file)+'.tif', prd_im_fll.T, compress=9)

    scaled_prd_im_fll = scale(prd_im_fll)
    del prd_im_fll
    gc.collect()

    # mask_im = np.dstack([dataset_obj.get_mask().T]*3).astype('uint8')*255
    mask_im = np.dstack([TissueMaskGenerationPatch(im_im)]*3).astype('uint8')*255
    mask_im[:,:,0] = scaled_prd_im_fll*255
    imsave(scaled_count_map,mask_im,prob_map,scaled_prd_im_fll,im_im,out=os.path.join(out_path,'ref_'+out_file)+'.png')
